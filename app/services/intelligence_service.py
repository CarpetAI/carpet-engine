...
def generate_action_id_with_llm(parsed_events: List[Dict[str, Any]], project_id: str = None, batch_size: int = 10) -> List[str]:
    """
    ...
    try:
        ...
        for i in range(0, len(parsed_events), batch_size):
            ...
            APPLOGGER.info(f"Processing batch {batch_start}-{batch_end} of {len(parsed_events)} events")
            ...
            response = client.chat.completions.create(
                model="gpt-4.1",
                messages=[
                    {"role": "system", "content": "You are an expert at analyzing user interactions and generating meaningful action descriptions. You prioritize reusing existing action IDs when they match the user's intent."},
            ...
            try:
                import json
                batch_action_ids = json.loads(result)
                
                if isinstance(batch_action_ids, list) and len(batch_action_ids) == len(batch_events):
                    for action_id in batch_action_ids:
                        all_action_ids.add(action_id)
                    
                    all_action_ids_list.extend(batch_action_ids)
                    APPLOGGER.info(f"Successfully generated {len(batch_action_ids)} action IDs for batch {batch_start}-{batch_end}")
                else:
                    APPLOGGER.error(f"Invalid response format for batch {batch_start}-{batch_end}: expected {len(batch_events)} action IDs, got {len(batch_action_ids) if isinstance(batch_action_ids, list) else 'non-list'}")
                    fallback_ids = [f"clicked_element_{j}" for j in range(len(batch_events))]
                    all_action_ids_list.extend(fallback_ids)
                    
            except json.JSONDecodeError as e:
                APPLOGGER.error(f"Failed to parse LLM response as JSON for batch {batch_start}-{batch_end}: {e}")
                fallback_ids = [f"clicked_element_{j}" for j in range(len(batch_events))]
                all_action_ids_list.extend(fallback_ids)
        
        APPLOGGER.info(f"Generated {len(all_action_ids_list)} total action IDs for {len(parsed_events)} events")
...
def generate_event_log_from_events(parsed_events: List[Dict[str, Any]], session_id: str, project_id: str = None, batch_size: int = 10) -> List[Dict[str, Any]]:
    """
    ...
    APPLOGGER.info(f"Returned {len(action_ids)} action IDs and parsed events length is {len(parsed_events)}")
